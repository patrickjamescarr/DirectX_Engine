...
bool Shader::InitStandard(ID3D11Device * device, WCHAR * vsFilename, WCHAR * psFilename)
{
	...
	DX::ThrowIfFailed( device->CreatePixelShader(pixelShaderBuffer.data(), pixelShaderBuffer.size(), NULL, &m_pixelShader));
	//if (result != S_OK)
	//{
	//	//if loading failed. 
	//	return false;
	...
	// Create the constant buffer pointer so we can access the vertex shader constant buffer from within this class.
	DX::ThrowIfFailed(device->CreateBuffer(&lightBufferDesc, NULL, &m_lightBuffer));
