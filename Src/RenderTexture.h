...
public:
	...
	*/
    RenderTexture(ID3D11Device* device, int textureWidth, int textureHeight, ID3D11DepthStencilView* depthStencilView);
	RenderTexture(ID3D11Device* device, int textureWidth, int textureHeight, float screenNear, float screenDepth);

    void ConfigureRenderTexture(ID3D11Device * device);

    void ConfigureViewport();

    void ConfigureProjectionMatrices(float screenNear, float screenFar);
	...
	ID3D11ShaderResourceView* getShaderResourceView() const;			///< Get the data from this render target as a texture resource.
...
private:
    void ConfigureDepthBufferAndStencil(ID3D11Device * device);
private:
